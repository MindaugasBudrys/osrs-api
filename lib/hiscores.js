...
const createHiscoreUrl = (playerType) =>
  `http://services.runescape.com/m=hiscore_oldschool${playerType == "normal" ? "" : `_${playerType}`}/index_lite.ws`;

const getPlayer = async (player) => {
  const hiscoreResponse = await axios.get(createHiscoreUrl(player.type), {
    params: { player: player.name }
  });

  return appendPlayer(mapPlayerStats(csvTo2dArray(hiscoreResponse.data)), player);
};

const getPlayers = async (players) => {
  return await Promise.all(players.map(getPlayer));
};
